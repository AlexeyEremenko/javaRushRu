...
public class Room {
    ...
    public void print() {
        ...
        int counter = 0;
        for (SnakeSection section : snake.getSections()) {//Рисуем все кусочки змеи
            if (counter == 0) {frame[section.getX()][section.getY()] = 2;}
            if (counter >= 1) {frame[section.getX()][section.getY()] = 1;}
            counter++;
        }

        frame[mouse.getX()][mouse.getY()] = 3;//Рисуем мышь

        for (int i = 0; i < width; i++) {//Выводим все это на экран
            for (int j = 0; j < height; j++) {
                if (frame[i][j] == 0) {System.out.print(".");}
                if (frame[i][j] == 2) {System.out.print("X");}
                if (frame[i][j] == 1) {System.out.print("x");}
                if (frame[i][j] == 3) {System.out.print("^");}
            }
            System.out.println();
        }
    }
